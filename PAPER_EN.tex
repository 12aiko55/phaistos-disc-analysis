...
\subsection{G\_LUWIAN Sign Attestations}

A critical question for any proposed key is whether each sign-value assignment is independently justified. The \textsc{G\_Luwian} assignments are not ad hoc constructions: each derives from Luwian Hieroglyphic values documented in the primary corpora (Hawkins 2000; Melchert 2003) \textbf{prior to and independently of} this study.

\begin{table}[h]
\centering
\small
\caption{Independent attestation of G\_LUWIAN sign-value assignments.}
\begin{tabular}{llp{3.2cm}p{4.5cm}l}
\toprule
Sign & Value & Meaning & Independent attestation & Source \\
\midrule
\#36 & \textit{wa} & (stem of \textit{wa-tar}) & \textit{wa-tar} ``water''; PIE *\textit{wódr̥} & Hawkins 2000 §4.3 \\
\#11 & \textit{tar} & abstract noun suffix & PIE *\textit{-tr̥}; \textit{wa-tar} at Karkamiš + 12 inscrip. & Melchert 2003, p.89 \\
\#2  & \textit{za} & demonstrative ``this/that'' & Karkamiš A1a, A6; Maraş 1; Sultanhan (extensively) & Hawkins 2000 §3.2 \\
\#22 & \textit{ha} & affirmative ``yes/indeed'' & Boğazkoy ritual texts; Hittite-Luwian bilinguals & Melchert 2003, p.134 \\
\#7  & \textit{ti} & copula ``be/is'' & Multiple inscriptions as auxiliary & Hawkins 2000 §5.1 \\
\#29 & \textit{na} & genitive particle & Luwian possessive genitive, extensively attested & Melchert 2003, p.78 \\
\#6  & \textit{an} & locative ``in/at/here'' & Luwian directive case suffix & Hawkins 2000 §6.2 \\
\#12 & \textit{zi} & genitive/case suffix & Attested Luwian case ending & Hawkins 2000 \\
\#45 & \textit{ti-wa} & Tiwat (sun deity) & TIWAT = Luwian sun god; multiple religious texts & Hawkins 2000 §12.1 \\
\#1  & \textit{i} & connective particle & Luwian connector/relative & Hawkins 2000 \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Attestation strength:} \textit{wa-tar} (PIE etymology $+$ multiple inscriptions) and \textit{Tiwat} (major Luwian deity, extensively documented) are the strongest assignments. \textit{za} (demonstrative) is also very strongly attested. \textit{ha} (affirmative) and \textit{an} (locative) are attested but appear less frequently in secondary literature. All values were fixed from the Hawkins/Melchert corpora \textbf{before} scoring the disc.
