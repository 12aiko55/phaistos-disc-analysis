...
### 6.25.8 CLL Lexical Validation: Direct Headword Match

As an independent post-hoc validation of the Algorithm #5 inferred key, we performed a systematic extraction of all relevant entries from Melchert's *Cuneiform Luwian Lexicon* (CLL, 1993) — the authoritative lexicographic reference for Cuneiform Luwian. The full 309-page PDF was processed programmatically to locate every CLL headword beginning with each of the 28 disc syllables (see `phaistos_translate.py`).

**Primary finding: *tiwali(ya)-* 'of the Sun-god' (CLL p. 239).** The three inferred sign values *ti* (#07, G\_LUWIAN known), *wa* (#36, G\_LUWIAN known), and *li* (#08, Algorithm #5 medium confidence) concatenate to produce the syllabic string `tiwali` — which is the exact onset of the CLL headword *tiwali(ya)-*, an adjective meaning "of / belonging to the Sun-god (*DTiwat-*)" formed by the suffix *-li(ya)-* on the solar deity name. The disc signs #07·#36·#08 (*ti·wa·li*) appear together in this order in **six word-groups**: A03, A21, A22, B06, B21, B22. In each case the greedy morpheme-lookup algorithm returns `tiwali(ya)- 'of the Sun-god'` as the three-syllable match, in preference to the two-syllable substring `wali 'dies'`. This is the **first instance in this study of a disc sign-sequence matching an attested CLL headword directly** — without requiring any additional assumptions beyond the phonetic key.

A parallel two-syllable sequence `wa·li` (signs #36·#08) appears at word-end in four additional word-groups (A24, A25, B23, B24), where the three-syllable *tiwali* match is not available. There the algorithm returns `wali 'dies; dead'`, consistent with the CLL-attested participial stem *walant(i)- 'dead'* < *\*wal-* 'die' (CLL p. 260; cf. HLuv. *wa-la/i-*; Lyc. *la-* 'die').

**Corrected function-word readings.** The CLL extraction also corrects six function-word glosses used in earlier versions of this paper's translation attempt:

| Sign | Value | Prior gloss | CLL-corrected gloss | CLL ref |
|------|-------|-------------|---------------------|---------|
| #22 | ha | "affirmative particle" | **and; also** (copulative clitic *-æa*) | p. 55 |
| #29 | na | "connective and" | **not** (negation *nā*, with ?) | p. 162 |
| #31 | la | "negation lā-" | **take** (verb *lā-*) | p. 130 |
| #13 | ma | "but; however" | **if, when(ever)** (*mān*, conditional) | p. 141 |
| #07 | ti | "conditional if" | **you** (2sg pronoun) / *-ti* reflexive | p. 226 |
| #15 | u  | "connective or" | **drink** (*u-*) / **know** (*u-ni-*) | p. 251 |

These corrections shift the interpretation of several recurring disc formulas. Notably, *ha* (#22) = *-æa* "and; also" rather than an affirmative particle means the three CERTAIN word-groups now read:

- **A27/B26** `za·tar·tiwa·na·ti`: "this — [delivers] — Sun-god — not — you" *or* "... and [then] you" (na ambiguous between CLL negation and connective function)
- **A31/B30** `za·wa·na·ti·i`: "this water — not — you — go" (prohibition) *or* "... and then you go" (invitation)
- **B05** `za·ha·ti·i`: "this — **and** — you — go" (copulative formula; *ha* = copulative, not affirmative)

> ⚠ The CLL is based on Cuneiform Luwian texts dated ca. 1400–1200 BCE, approximately 300 years later than the disc (~1700 BCE). Lexical forms may differ. All CLL-backed glosses are provisional and require review by a specialist in Cuneiform and Hieroglyphic Luwian.
...
## Appendix B: CLL-Backed Provisional Translation (All 61 Word-Groups)

> ⚠ **Epistemic status: hypothesis-level only.** This appendix presents a provisional morpheme-by-morpheme reading of all 61 disc word-groups using the G\_LUWIAN phonetic key (Achterberg 2004, 10 known signs) combined with Algorithm #5 inferred values (18 signs) and a lexicon extracted directly from Melchert's *Cuneiform Luwian Lexicon* (CLL, 1993, 309 pp.). The CLL is based on Cuneiform Luwian texts ca. 1400–1200 BCE, approximately 300 years after the disc (~1700 BCE). All readings require validation by a specialist in Cuneiform and Hieroglyphic Luwian before any scholarly claim can be based on them.
>
> **Confidence tiers:** CERTAIN = all G\_LUWIAN known signs; provisional = HIGH-confidence inferred signs only; tentative = includes MED-confidence signs; speculative = includes LOW-confidence signs.
>
> **Key correction from prior versions:** *ha* (#22) = *-æa* "and; also" (CLL p. 55), not "affirmative". *na* (#29) = *nā* "not" (CLL p. 162). *la* (#31) = *lā-* "take" (CLL p. 130). *ti* (#07) = "you (2sg)" or reflexive (CLL p. 226).
>
> **Primary finding:** Signs #07·#36·#08 (*ti·wa·li*) spell the CLL adjective *tiwali(ya)-* "of the Sun-god" (CLL p. 239) in **6 word-groups** (A03, A21, A22, B06, B20, B21). Signs #36·#08 (*wa·li*) alone = *wali* "dies" (CLL p. 260) in 4 further groups (A24, A25, B23, B24).

Generated by `phaistos_translate.py` (CLL-backed lexicon, greedy 4→3→2→1 syllable matching).

### Side A (outside → center)

| Word | Phonetic reading | CLL morpheme analysis | Conf |
|------|-----------------|----------------------|------|
| **A01** | `za·zi·ma·i·ri` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *i*=go + *ri*=? | tentative |
| **A02** | `za·zi·ma·ia·i·ri` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *ia*=? + *i*=go + *ri*=? | tentative |
| **A03** | `za·ti·wa·li·i·ku` | *za*=this + ***tiwali*=of the Sun-god** + *i*=go + *ku*=? | tentative |
| **A04** | `za·an·zi·ku·nu·i·in` | *za*=this + *anzi*=us/3pl + *ku*=? + *nu*=and + *i*=go + *in*=? | tentative |
| **A05** | `za·as·zi·ku·nu·i·in` | *za*=this + *as*=? + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go + *in*=? | tentative |
| **A06** | `za·ah·zi·ku·nu·i·in` | *za*=this + *ah*=? + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go + *in*=? | speculative |
| **A07** | `za·ti·zi·u·wa·ku` | *za*=this + *ti*=you + *zi*=lie/subsequently + *u*=drink + *wa*=? + *ku*=? | tentative |
| **A08** | `za·zi·ku·nu·i·in` | *za*=this + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go + *in*=? | tentative |
| **A09** | `za·zi·u·wa·ku` | *za*=this + *zi*=lie/subsequently + *u*=drink + *wa*=? + *ku*=? | tentative |
| **A10** | `za·zi·ku·nu·i` | *za*=this + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go | tentative |
| **A11** | `za·si·an·zi·la` | *za*=this + *si*=himself + *anzi*=us/3pl + *la*=take | provisional |
| **A12** | `za·an·zi·la` | *za*=this + *anzi*=us/3pl + *la*=take | provisional |
| **A13** | `za·si·in·zi·ku·i·ta` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ta*=? | tentative |
| **A14** | `za·zi·ku·i·ta` | *za*=this + *zi*=lie/subsequently + *ku*=? + *i*=go + *ta*=? | tentative |
| **A15** | `za·si·in·zi·ku·i·ri` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? | tentative |
| **A16** | `za·in·zi·ku·i·ri` | *za*=this + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? | tentative |
| **A17** | `za·si·in·zi·ku·i·ri·im` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? + *im*=indeed | tentative |
| **A18** | `za·zi·ku·i·ri·im` | *za*=this + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? + *im*=indeed | tentative |
| **A19** | `za·si·in·zi·ku·i·ru` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ru*=? | speculative |
| **A20** | `za·zi·ku·i·ru` | *za*=this + *zi*=lie/subsequently + *ku*=? + *i*=go + *ru*=? | speculative |
| **A21** | `za·in·zi·ti·wa·li` | *za*=this + *in*=? + *zi*=lie/subsequently + ***tiwali*=of the Sun-god** | tentative |
| **A22** | `za·zi·ti·wa·li` | *za*=this + *zi*=lie/subsequently + ***tiwali*=of the Sun-god** | tentative |
| **A23** | `za·zi·ma·a·i·wa` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *a*=when + *i*=go + *wa*=? | provisional |
| **A24** | `za·zi·ma·a·i·wa·li` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *a*=when + *i*=go + ***wali*=dies** | tentative |
| **A25** | `za·zi·ma·a·i·wa·li·im` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *a*=when + *i*=go + ***wali*=dies** + *im*=indeed | tentative |
| **A26** | `za·zi·ha·ti·ti·is` | *za*=this + *zi*=lie/subsequently + *hati*=and-you + *ti*=you + *is*=? | provisional |
| **A27** ★ | `za·tar·tiwa·na·ti` | *za*=this + *tar*=hand-over + *tiwa*=Sun-god + *nati*=not-you | **CERTAIN** |
| **A28** | `za·na·ti·ti·ia` | *za*=this + *nati*=not-you + *ti*=you + *ia*=? | provisional |
| **A29** | `za·wa·zi·ku·nu·i` | *zawa*=this-water + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go | tentative |
| **A30** | `za·na·is·tu·u` | *za*=this + *na*=not + *istu*=from + *u*=drink | provisional |
| **A31** ★ | `za·wa·na·ti·i` | *zawa*=this-water + *nati*=not-you + *i*=go | **CERTAIN** |

### Side B (center → outside)

| Word | Phonetic reading | CLL morpheme analysis | Conf |
|------|-----------------|----------------------|------|
| **B01** | `za·zi·ma·i·tar·tiwa` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *i*=go + *tar*=hand-over + *tiwa*=Sun-god | provisional |
| **B02** | `za·zi·ma·i·wa·tiwa` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *i*=go + *wa*=? + *tiwa*=Sun-god | provisional |
| **B03** | `za·zi·ma·i·ri` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *i*=go + *ri*=? | tentative |
| **B04** | `za·ha·na·ri` | *za*=this + *ha*=and + *na*=not + *ri*=? | tentative |
| **B05** ★ | `za·ha·ti·i` | *za*=this + *hati*=and-you + *i*=go | **CERTAIN** |
| **B06** | `za·ti·wa·li·i·ku` | *za*=this + ***tiwali*=of the Sun-god** + *i*=go + *ku*=? | tentative |
| **B07** | `za·zi·u·wa·ku` | *za*=this + *zi*=lie/subsequently + *u*=drink + *wa*=? + *ku*=? | tentative |
| **B08** | `za·zi·ku·nu·i·in` | *za*=this + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go + *in*=? | tentative |
| **B09** | `za·zi·ku·nu·i` | *za*=this + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go | tentative |
| **B10** | `za·si·an·zi·la` | *za*=this + *si*=himself + *anzi*=us/3pl + *la*=take | provisional |
| **B11** | `za·an·zi·la` | *za*=this + *anzi*=us/3pl + *la*=take | provisional |
| **B12** | `za·si·in·zi·ku·i·ta` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ta*=? | tentative |
| **B13** | `za·zi·ku·i·ta` | *za*=this + *zi*=lie/subsequently + *ku*=? + *i*=go + *ta*=? | tentative |
| **B14** | `za·si·in·zi·ku·i·ri` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? | tentative |
| **B15** | `za·in·zi·ku·i·ri` | *za*=this + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? | tentative |
| **B16** | `za·si·in·zi·ku·i·ri·im` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? + *im*=indeed | tentative |
| **B17** | `za·zi·ku·i·ri·im` | *za*=this + *zi*=lie/subsequently + *ku*=? + *i*=go + *ri*=? + *im*=indeed | tentative |
| **B18** | `za·si·in·zi·ku·i·ru` | *za*=this + *si*=himself + *in*=? + *zi*=lie/subsequently + *ku*=? + *i*=go + *ru*=? | speculative |
| **B19** | `za·zi·ku·i·ru` | *za*=this + *zi*=lie/subsequently + *ku*=? + *i*=go + *ru*=? | speculative |
| **B20** | `za·in·zi·ti·wa·li` | *za*=this + *in*=? + *zi*=lie/subsequently + ***tiwali*=of the Sun-god** | tentative |
| **B21** | `za·zi·ti·wa·li` | *za*=this + *zi*=lie/subsequently + ***tiwali*=of the Sun-god** | tentative |
| **B22** | `za·zi·ma·a·i·wa` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *a*=when + *i*=go + *wa*=? | provisional |
| **B23** | `za·zi·ma·a·i·wa·li` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *a*=when + *i*=go + ***wali*=dies** | tentative |
| **B24** | `za·zi·ma·a·i·wa·li·im` | *za*=this + *zi*=lie/subsequently + *ma*=if/when + *a*=when + *i*=go + ***wali*=dies** + *im*=indeed | tentative |
| **B25** | `za·zi·ha·ti·ti·is` | *za*=this + *zi*=lie/subsequently + *hati*=and-you + *ti*=you + *is*=? | provisional |
| **B26** ★ | `za·tar·tiwa·na·ti` | *za*=this + *tar*=hand-over + *tiwa*=Sun-god + *nati*=not-you | **CERTAIN** |
| **B27** | `za·na·ti·ti·ia` | *za*=this + *nati*=not-you + *ti*=you + *ia*=? | provisional |
| **B28** | `za·wa·zi·ku·nu·i` | *zawa*=this-water + *zi*=lie/subsequently + *ku*=? + *nu*=and + *i*=go | tentative |
| **B29** | `za·na·is·tu·u` | *za*=this + *na*=not + *istu*=from + *u*=drink | provisional |
| **B30** ★ | `za·wa·na·ti·i` | *zawa*=this-water + *nati*=not-you + *i*=go | **CERTAIN** |

### Summary: Recurring semantic clusters

| Pattern | Occurrences | CLL reading | Interpretation |
|---------|-------------|-------------|----------------|
| `ti·wa·li` (signs #07·#36·#08) | ×6 | *tiwali(ya)-* "of the Sun-god" | Solar invocation formula |
| `wa·li` (signs #36·#08) | ×4 | *wali* "dies; dead" | Death/descent reference |
| `an·zi·la` (signs #06·#12·#31) | ×4 | *anzi* "us" + *la* "take" | "Us take" — offering receipt? |
| `za·wa·na·ti·i` (certain) | ×2 | "this water — not you — go" | Water prohibition/invitation |
| `za·tar·tiwa·na·ti` (certain) | ×2 | "this — delivers — Sun-god — not-you" | Solar covenant formula |
| `zi·ma·a·i` | ×6 | "subsequently — if/when — when — go" | Conditional movement clause |
